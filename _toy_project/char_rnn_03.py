...
# 학습에 필요한 설정값들을 지정합니다.
# data_dir = 'D:\study_data\_data\project\quote_generator/input_label'
data_dir = 'D:\study_data\_data\project\quote_generator/input_nolabel'
batch_size = 100 # Training : 50, Sampling : 1
seq_length = 30 # Training : 50, Sampling : 1
hidden_size = 30   # 히든 레이어의 노드 개수
learning_rate = 0.002
num_epochs = 100
...
with tf.Session() as sess:
    ...
    # 샘플링 시작
    print("샘플링을 시작합니다!")
    num_sampling = 25  # 생성할 글자(Character)의 개수를 지정합니다. 
    prime = u'인생 '         # 시작 글자를 ' '(공백)으로 지정합니다.
...
# saver = tf.train.import_meta_graph('./my_model/my_model-1000.meta')      # network 불러오기

# saver.restore(sess, tf.train.latest_checkpoint('./'))                    # 가장 마지막 checkpoint
...
# W = tf.Variable(tf.random_normal([1]), name='weight')    # 저장할 W 생성해야 함.(중요)
