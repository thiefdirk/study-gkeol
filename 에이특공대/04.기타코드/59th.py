...
x = data
x = data.drop(['PIA205B-02A_MIN', 'PIA205B-02A_MAX'], axis=1)   #종속변수
...
model = Sequential()
model.add(Conv1D(filters=128, kernel_size=2, padding='same', input_shape=(6, 17)))
...
history = model.fit(x_train, y_train, epochs=1300, batch_size=256, validation_split=0.2, verbose=1, callbacks=[es,reduce_lr])
