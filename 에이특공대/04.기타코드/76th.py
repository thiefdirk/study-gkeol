...
test_out_data = test_out_data.drop(test_out_data.columns[0], axis=1)
...
# exit()
########################################
# 컬럼드롭 고려대상: ti-mean, temp, ground, min, max
x = data
# x = x.drop(['PIA205B-02A_MIN', 'PIA205B-02A_MAX'], axis=1)   #종속변수
x = x.drop(['STN-MFR-S','LP_TOTAL', 'PRESSURE-S','FI_SUM','PIA205B-02A_MIN'], axis=1)   #독립변수
test_sets = test_sets.drop(['STN-MFR-S','LP_TOTAL', 'PRESSURE-S','FI_SUM','PIA205B-02A_MIN'], axis=1)
...
history = model.fit(x_train, y_train, epochs=1000, batch_size=256, validation_split=0.2, verbose=1, callbacks=[es,reduce_lr,cp])
