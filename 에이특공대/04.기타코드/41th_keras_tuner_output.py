...
model = Sequential()
model.add(Conv1D(filters=224, kernel_size=1, padding='same', input_shape=(6, 12)))
model.add(MaxPooling1D(pool_size=1))
# model.add(Conv1D(filters=64, kernel_size=2, padding='same'))
# model.add(MaxPooling1D(pool_size=2))
model.add(Conv1D(filters=104, kernel_size=1, padding='same'))
model.add(Flatten())
model.add(Dense(62, activation='selu'))
model.add(Dense(47, activation='selu'))
