...
model = Sequential()
model.add(LSTM(units=232, return_sequences=True, input_shape=(6,19)))
model.add(LSTM(units=208, return_sequences=True))
...
model.fit(x_train, y_train, epochs=200, batch_size=1024, validation_data=(x_test, y_test), verbose=1, callbacks=[es,reduce_lr])
