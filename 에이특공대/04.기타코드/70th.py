...
'''
[1] 외부데이터
print(out_data.head())  #  temp   hum  sun  ground  wind  rain  snow
print(test_out_data.head()) # temp   hum  sun  ground  wind  rain  snow
외부데이터 총 7개 사용 (temp, hum, sun, ground, wind, rain, snow) 및 데이터 잘 불러와졌는지 상단 5줄을 통해 확인

[2] 파생피처 생성
'bog'='FY_SUM' + 'FIA_SUM'
'lp_ti' = 'FY_SUM' + 'LP_TOTAL'
기타 등등,,,
시계열 데이터 파생피처 년, 월, 일,,
...
[5] 삭제 후 컬럼 00개, 이상치 확인

[6] 컬럼별 이상치 상이, 이에따라 컬럼 별 전처리 각각 진행

[7] scaling 후 이상치 재확인 (스케일링 후 범위는 00~00 사이로 축소)

[8] split

[9] model 구성 및 훈련과정 시각화
- 모델 구성, 훈련, 평가 등등
