...
test_sets = test_sets.drop(['PIA205B-02A_MIN', 'PIA205B-02A_MAX'], axis=1)
...
# x_reshaped = np.reshape(x, (109722,108,1))
# x_flatten = x.flatten()
# print(x_flatten.shape)

x_reshaped = np.reshape(x, (109722,96))
test_sets_reshaped = np.reshape(test_sets, (5508,96))
...
cat1.fit(x_reshaped, y_min)

cat2.fit(x_reshaped, y_max)


min_predict = cat1.predict(test_sets_reshaped)

max_predict = cat2.predict(test_sets_reshaped)
...
pickle.dump(cat1, open(f'C:/Users/kogas/Desktop/ai_factory/_save/Model/{try_num}th_cat_min.pkl', 'wb'))
pickle.dump(cat2, open(f'C:/Users/kogas/Desktop/ai_factory/_save/Model/{try_num}th_cat_max.pkl', 'wb'))


submission.to_csv(f'C:/Users/kogas/Desktop/ai_factory/_data/dataset/submission_{try_num}th_cat.csv')
...
history = model.fit(x_train, y_train, epochs=1000, batch_size=256, validation_split=0.2, verbose=1, callbacks=[es,reduce_lr])
