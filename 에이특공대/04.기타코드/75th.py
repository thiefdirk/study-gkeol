...
# from tensorflow.python.keras.layers import Dense, LSTM, Dropout, Conv1D, Flatten, MaxPooling1D, GlobalAveragePooling1D,GlobalMaxPooling1D ,ReLU
from tensorflow.keras.models import load_model
from tensorflow.keras.layers import BatchNormalization, Dense, LSTM, Dropout, Conv1D, Flatten, MaxPooling1D, GlobalAveragePooling1D,GlobalMaxPooling1D ,ReLU
...
oh_encord3 = pd.get_dummies(data['month'])
data = pd.concat([data, oh_encord3], axis=1)

oh_encord4 = pd.get_dummies(test_sets['month'])
oh1test = pd.DataFrame(0,index=range(len(oh_encord4)), columns = {1}, dtype=np.uint8)
oh2test = pd.DataFrame(0,index=range(len(oh_encord4)), columns = {2}, dtype=np.uint8)
oh12test = pd.concat([oh1test, oh2test], axis=1)
oh123test = pd.concat([oh12test, oh_encord4], axis=1)
test_sets = pd.concat([test_sets, oh123test], axis=1)
...
strd_list = ['FY_SUM','wind', 'rain', 'snow', 'lp_ti', 'bog']
strd_list2 = ['FIA_SUM']
norm_list = ['weekofyear', 'TI_MEAN', 'temp', 'hum', 'sun']


data = data.drop(['TIME','TIME2', 'weekday','year','dayofyear','month'], axis=1, inplace=False)

test_sets = test_sets.drop(['TIME','TIME2', 'weekday','year','dayofyear','month'], axis=1, inplace=False)
...
x_test, x_valid, y_test, y_valid = train_test_split(x_test, y_test, train_size=0.05, shuffle=False, random_state=66)

model = Sequential()
model.add(Conv1D(filters=256, kernel_size=2, padding='same', input_shape=(6, 31)))
...
history = model.fit(x_train, y_train, epochs=3000, batch_size=512, validation_data=(x_valid, y_valid), verbose=1, callbacks=[es,reduce_lr,cp])
