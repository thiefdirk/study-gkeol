...
test_x = pd.read_csv(test_sets_path + 'test_0.csv', header=0)
test_x = test_x[:-1]

for i in range(5507):
    test_set = pd.read_csv(test_sets_path + f'test_{i+1}.csv', header=0)
    test_set = test_set[:-1]
    test_x = pd.concat([test_x, test_set], axis=0)
    
test_y = pd.read_csv(test_sets_path + 'test_0.csv', header=0)
test_y = test_y[1:]

for i in range(5507):
    test_set = pd.read_csv(test_sets_path + f'test_{i+1}.csv', header=0)
    test_set = test_set[1:]
    test_y = pd.concat([test_y, test_set], axis=0)
...
test_x = test_x.loc[:,['PIA205B-02A_MIN', 'PIA205B-02A_MAX']]
test_y = test_y.loc[:,['PIA205B-02A_MIN', 'PIA205B-02A_MAX']]
...
bayesian_paragram = {
                    # 'learning_rate' : (0.01, 1.0),
                     ...
                     'max_leaves' : (10, 300),
                    #  'min_split_loss' : (1e-9, 100),
                    #  'reg_alpha' : (1e-9, 100),
                     'n_estimators' : (50, 500)
                     }

def xgb_fuc(max_depth, subsample, colsample_bytree, max_leaves, n_estimators):
    params = {
...
score = BayesianOptimization(xgb_fuc, bayesian_paragram, random_state=666, verbose=2)
score.maximize(init_points=25, n_iter=100, acq = 'ei', xi = 0.01)
print(score.max)
exit()
submission_predict = model.predict(test_sets)
