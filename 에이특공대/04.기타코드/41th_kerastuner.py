...
def build_model(hp):
    model = Sequential()
    model.add(Conv1D(filters=hp.Int('filters1', min_value=32, max_value=512, step=32), kernel_size=hp.Int('kernel_size1', min_value=1, max_value=10, step=32), padding='same', input_shape=(6, 12)))
    model.add(MaxPooling1D(pool_size=hp.Int('pool_size1', min_value=1, max_value=6, step=32)))
    # model.add(Conv1D(filters=64, kernel_size=2, padding='same'))
    # model.add(MaxPooling1D(pool_size=2))
    model.add(Conv1D(filters=hp.Int('filters2', min_value=8, max_value=128, step=32), kernel_size=hp.Int('kernel_size2', min_value=1, max_value=10, step=32), padding='same'))
    model.add(Flatten())
    model.add(Dense(hp.Int('unit1', min_value=30, max_value=200, step=32), activation='selu'))
    model.add(Dense(hp.Int('unit2', min_value=15, max_value=100, step=32), activation='selu'))
    model.add(Dense(hp.Int('unit3', min_value=7, max_value=50, step=32), activation='relu'))
