...
x_reshaped = pd.read_csv('C:/Users/kogas/Desktop/ai_factory/x_reshaped.csv', header=0, names=['ZIH120-02','PIA205B-02A_MIN','PIA205B-02A_MAX','FI_SUM','FY_SUM','TI_MEAN','FIA_SUM'])
test_sets_reshaped = pd.read_csv('C:/Users/kogas/Desktop/ai_factory/test_sets_reshaped.csv', header=0, names=['ZIH120-02','PIA205B-02A_MIN_','PIA205B-02A_MAX_','FI_SUM','FY_SUM','TI_MEAN','FIA_SUM'])
...
def np_to_df(np_array):
    df = pd.DataFrame(np_array[0])
    for i in range(1, len(np_array)):
        df = df.append(pd.DataFrame(np_array[i]))
    return df

# x = np_to_df(x)
# y = np_to_df(y)

# test_set = np_to_df(test_set)
...
y = pd.DataFrame(y, columns=['PIA205B-02A_MIN', 'PIA205B-02A_MAX'])
all_data = pd.concat([x_reshaped, y], axis=1)
all_data['residual'] = all_data['PIA205B-02A_MIN']
...
setup = setup(session_id = 777, data=all_data, target=('PIA205B-02A_MIN', 'PIA205B-02A_MAX'), train_size = 0.85, normalize = True, normalize_method='zscore', transformation=True, use_gpu=True)
