...
# pca = PCA() # n_components : 주요하지 않은 변수를 제거하고 싶은 개수를 지정한다.
# x = pca.fit_transform(train_set) # x를 pca로 변환한다.
# lda = LDA() # n_components : 주요하지 않은 변수를 제거하고 싶은 개수를 지정한다.
# x = lda.fit_transform(train_set, test_set)
...
# model = SVC()
model = make_pipeline(MinMaxScaler(), XGBClassifier(tree_method='gpu_hist', predictor='gpu_predictor', gpu_id=0, n_jobs=1,)) # pipeline을 사용하면 여러개의 모델을 한번에 학습시키기 때문에 성능이 좋아진다.
...
# model.fit 시간 :  0.5017349720001221

# XGBClassifier PCA // n_components : none
# model.score :  0.8268156424581006
# model.fit 시간 :  0.49329447746276855

# XGBClassifier LDA // n_components : none
# model.score :  0.776536312849162
# model.fit 시간 :  0.31375956535339355
