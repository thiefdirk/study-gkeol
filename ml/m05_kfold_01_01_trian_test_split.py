from os import access # access는 파일이 존재하는지 확인하는 함수
import numpy as np
...
n_split = 5
kfold = KFold(n_splits=n_split, shuffle=True, random_state=66) # kfold 
...
#3,4. 컴파일, 훈련, 평가, 예측
scores = cross_val_score(model, x_train, y_train, cv=kfold) # cross_val_score를 통해 스코어를 보여준다.
